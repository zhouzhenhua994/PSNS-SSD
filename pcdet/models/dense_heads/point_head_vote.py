...
class SG1(nn.Module):
    ...
    def __init__(self, groups):
        ...
        self.weight=nn.Parameter(torch.zeros(1,groups,1).cuda())
        self.bias=nn.Parameter(torch.zeros(1,groups,1).cuda())
        self.sig=nn.Sigmoid()
        self.init_weights()
    ...
    def forward(self, x):
        x1 = x
        b, c, h = x.shape
        x = x.reshape(b * self.groups, -1, h)  # bs*g,dim//g,h,w
        ...
        t = t / std  # bs*g,h*w
        t = t.reshape(b, self.groups, h)  # bs,g,h*w

        t = t * self.weight + self.bias  # bs,g,h*w
        t = t.reshape(b * self.groups, 1, h)  # bs*g,1,h*w
        x = x * self.sig(t)
        x = x.reshape(b, c, h)
...
class PointHeadVote(PointHeadTemplate):
    ...
    def __init__(self, num_class, input_channels, model_cfg, predict_boxes_when_training=False, **kwargs):
        ...
        fc_layers2.append(nn.Conv1d(128, 3, kernel_size=1, bias=True))
    ...
    def forward(self, batch_dict):
        ...
        features_select = candidate_features
        new_features = self.fc_layers(candidate_features)

        ##G2## +
        global_features = torch.cat((new_features, features_select), 1)  # (B, 256+128, 1)

        c1 = nn.Linear(384, 128, bias=False, device="cuda")
        global_features = global_features.transpose(1, 2)
        global_features = c1(global_features)
        global_features = global_features.transpose(1, 2)

        se = SG1(groups=8)
        global_features = se(global_features)

        b1 = nn.BatchNorm1d(global_features.size(1), device="cuda")
        global_features = b1(global_features)
        r1 = nn.ReLU()
        global_features = r1(global_features)

        new_features = new_features + global_features

        r2 = nn.ReLU()
        vote_offsets = r2(new_features)

        vote_offsets = self.fc_layers2(vote_offsets)

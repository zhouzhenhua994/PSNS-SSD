...
class PointNet2FSMSG(nn.Module):
    def __init__(self, model_cfg, input_channels, **kwargs):
        ...
        self.confidence_mlps2 = self.model_cfg.SA_CONFIG.get('CONFIDENCE_MLPS2', None)
        self.pre_aggregation_mlps = self.model_cfg.SA_CONFIG.get('PRE_AGGREGATION_MLPS', [False for _ in range(len(self.model_cfg.SA_CONFIG.NPOINT_LIST))])
        ...
        for k in range(self.model_cfg.SA_CONFIG.NPOINT_LIST.__len__()):
            ...
            # if self.confidence_mlps2 and self.confidence_mlps2[k]:
            #     confidence_mlp2 = self.confidence_mlps2[k].copy()
            #     if confidence_mlp2.__len__() == 0:
            #         confidence_mlp2 = None
            # else:
            #     confidence_mlp2 = None
    ...
    def forward(self, batch_dict):
        ...
        if li_cls_pred is not None:
            ...
            sa_ins_preds.append(torch.cat(
                [li_cls_batch_idx[..., None].float(), li_cls_pred.view(batch_size, -1, li_cls_pred.shape[-1])], dim=-1))
        else:
            sa_ins_preds.append([])
        ...
        batch_dict['sss3'] = l_xyz[3].squeeze(dim=0)
        batch_dict['sss2'] = l_xyz[2].squeeze(dim=0)
        batch_dict['sss1'] = l_xyz[2].squeeze(dim=0)
        batch_dict['sss0'] = l_xyz[2].squeeze(dim=0)
        return batch_dict
